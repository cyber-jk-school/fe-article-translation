...
예를 들어, 이 컴파일러는 잘 동작하지만 런타임에서 error을 발생시킵니다.
...
TypeScript에서는 평범한 관행이지만 실제로는 매우 위헙합니다.
...
그리고 이제 우리는 우리가 사용했던 일반적인 type을 기반으로 한 것과 반대로, 정확히 우리가 구성한 대로 IDE 의 자동완성과 모든 객체 트리에 대한 type을 체크할 수 있습니다.
